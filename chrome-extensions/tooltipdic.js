...
function gethoverDiv(event) {
    var hoverTipDiv;
    if (currDoc)
        hoverTipDiv = currDoc.getElementById("ToolTipDic");
...
function killHoverTip() {
    var hoverTipDiv = gethoverDiv();
    if (hoverTipDiv && hoverTipDiv.parentNode)
        hoverTipDiv.parentNode.removeChild(hoverTipDiv)
}
...
function getHoverText(event) {
    ...
    if (hoverTipDiv && ToolTipWord.indexOf(text) != -1)
        return false;
    killHoverTip();
    ...
    var top;
    if (boxPosition == "UP")
        top = (event.pageY - parseInt(offsetDistance) - hoverTipDiv.offsetHeight - dTop) / zoom;
    else
        top = (event.pageY + parseInt(offsetDistance) - dTop) / zoom;
...
function PositionCorrection(elem) {
    ...
    var top, left;
    if (boxPosition == "UP")
        top = (CursorY - parseInt(offsetDistance) - elem.offsetHeight) / zoom;
    else
        top = (CursorY + parseInt(offsetDistance)) / zoom;
    if (top < 0)
        top = 0;
    ...
    if (top + elem.offsetHeight > posBottom)
        top = posBottom - elem.offsetHeight;
    if (top < pYOffset)
        top = pYOffset;
    elem.style.setProperty("top", String(top) + "px", "important");
    if ((parseFloat(elem.style.left) + parseFloat(elem.offsetWidth)) * zoom > window.pageXOffset + viewportWidth)
        left = window.pageXOffset + viewportWidth - parseFloat(elem.offsetWidth) * zoom - 5;
    else
        left = CursorX;
