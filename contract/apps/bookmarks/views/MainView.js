define([
...
], function(_, Backbone, BaseView, PubSub, ModalView, tmpl, FolderModal) {
	...
	var BookmarkItem = BaseView.extend({
			...
			$el.on('click', function(e){
				...
				.on('submit', 'form', function(e){
					...
					$.ajax({
						...
						if(res.status){
							$el.find('._old_ .name').text(res.name);
							me.reset();
						}
					});
		...
		_moveBookmark: function(){
			...
			}).on('createdfolder', function() {
				me.trigger('changed');	
			}).on('deletedfolder', function() {
				me.trigger('changed');	
			});
			folder.open();			
		},
		...
		_deleteRow: function() {
			...
			if(!confirm(txt+'을(를) 삭제하시겠습니까?')){ return false; }
	...
	var MainView = BaseView.extend({
		...
		onFolderClick: function(e){
			...
			Backbone.history.navigate('/bookmarks?bm_id='+id, true);
		},
