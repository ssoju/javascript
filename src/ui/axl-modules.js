...
(function($, core, undefined) {
    ...
    var Accordion = ui('Accordion', /**@lends axl.ui.Accordion */ {
        ...
        collapse: function(index, isAni, cb) {
            ...
            // 열리기 전에 이벤트 발생
            if (me.triggerHandler('accordionbeforecollapse', data) === false) {
                return;
            }
            if (isAni !== false) {
                // 애니메이션 모드
        ...
        expand: function(index, isAni, cb) {
            ...
            if (me.triggerHandler('accordionbeforeexpand', data) === false) {
                return;
            }
            if (isAni !== false) {
                // 애니메이션 사용
    ...
    var Scrollview = core.ui('Scrollview', {
        ...
        _bindContentScroll: function() {
            var me = this,
                ...
                multiplier = 1,
                util = core.util,
                distance, startY, acc, scrollableY, wrapHeight, maxScrollY, startScrollTop;
            ...
            me.$el.on('touchstart touchmove touchend touchcancel', function(e) {
                ...
                switch (e.type) {
                    case 'touchstart':
                        ...
                        startY = util.getEventPoint(e).y;
                        multiplier = 1;

                        if (me.$wrapper.is(":animated") && (times['touchstart'] - times['touchend'] < me.options.accelerationT)) {
                    ...
                    case 'touchmove':
                        ...
                        if (me.options.preventScroll) {
                            ...
                            if (startY < pointY && startScrollTop === 0) {
                                return;
                            } else if (startY > pointY && startScrollTop === maxScrollY) {
                                return;
                            } else {
                                e.preventDefault();
                            }
...
(function($, core, undefined) {
    ...
    var Selectbox = core.ui('Selectbox', /** @lends axl.ui.Selectbox# */ {
        ...
        _create: function() {
            ...
            me.hideClass = (me.options.hideClass ? '.' + me.options.hideClass : ''); // TODO
            me.isReadonly = me.$el.prop('readonly') || !!~cls.indexOf('read');
            me.isDisabled = me.$el.prop('disabled') || !!~cls.indexOf('disabled');

            // 셀렉트박스
            me.$selectbox = $('<div class="ui_select_dom ' + cls + '" ' + elId + '></div>').addClass(me.options.wrapClasses);
        ...
        _refreshScroll: function() {
            ...
            me.wrapperHeight = me.$selectboxWrapper.height(); // - (me.options.containerMargin * 2);
            ...
            // TODO me.$scrollBar.css('marginTop', me.$label.height() + me.options.containerMargin);

            me.scrollRate = me.wrapperHeight / me.scrollerHeight;
            me.scrollBarHeight = me.wrapperHeight * me.scrollRate;
        ...
        _bindScrollEvent: function() {
                        ...
                        return function(type, data) {
                            ...
                            switch (type) {
                                case 'start':
                                    ...
                                    wrapperHeight = me.$selectboxWrapper.height();
                                    scrollHeight = me.$selectboxWrapper.prop('scrollHeight');
                                    break;
                                case 'move':
                                    if (!me.isMouseDown) {
                                        return;
                                    ...
                                    me.$selectboxWrapper.scrollTop(rate * (scrollHeight - wrapperHeight));
                                    break;
                                default:
                                    me.isMouseDown = false;
            ...
            var $listChild = me.$selectboxWrapper;
        ...
        _hideScroll: function() {
            ...
            me.$scrollBar.css({
                'height': 0,
                'top': 0
            }).find('span.bg_mid').css('height', 0);
        },

        _scrollUpdate: function() {
            var me = this;

            me._scrollbarMove(me.$selectboxWrapper.scrollTop() * me.scrollRate);
        },
        ...
        _scrollTop: function(top) {
            ...
            me.$selectboxWrapper.scrollTop(top * me.scrollRate);
            me._scrollUpdate();
        },
        ...
        _createList: function() {
            ...
            me.$selectboxWrapper = me.$list.find('>.ui_selectbox_wrapper');
            ...
            me.maxHeight = parseInt(me.$selectboxWrapper.css('max-height'), 10);
        ...
        _reposition: function() {
            ...
            listHeight = me.$selectboxWrapper.innerHeight();
        ...
        _scrollToItem: function() {
            var me = this,
                selIndex = me.$el[0].selectedIndex;

            if (selIndex > 0) {
                var $option = me.$list.find('li').eq(selIndex),
                    scrollTop = me.$selectboxWrapper.scrollTop(),
                ...
                if (optionTop < scrollTop || optionTop >= wrapperHeight + scrollTop) {
                    ...
                    listHeight = me.$selectboxWrapper.height();
                    me.$selectboxWrapper.scrollTop(optionTop - (listHeight / 2) + (optionHeight / 2));
                }
            } else {
                me.$selectboxWrapper.scrollTop(0);
            }
        },
        ...
        open: function() {
            ...
            if (me.$el.prop('disabled') || me.$el.prop('readonly')) {
                return;
            }
            me._reposition();
            ...
            me.$label.find('.ui_select_button').attr('title', me.attrTitle + ' 닫기');
            me._refreshScroll();
        },
        /**
        ...
        close: function() {
            ...
            me.$label.find('.ui_select_button').attr('title', me.attrTitle + ' 열기');
        ...
        _itemHTML: function(option, type) {
            ...
            if (cname = $o.attr('data-sup')) {
                dataList['sup'] = cname;
            }
            if (cname = $o.attr('data-cnum')) {
                dataList['cnum'] = cname;
            }
            if (cname = $o.attr('data-cname')) {
                dataList['cname'] = cname;
            }
            if (cname = $o.attr('data-cname-mobile')) {
                dataList['cnameMobile'] = cname;
            }
        ...
        selectedIndex: function(index, trigger) {
            ...
            if (this.isReadonly || this.isDisabled || this.el.options.length === 0) {
                return;
            }
            if (index < 0 || index >= this.$el[0].options.length) {
                return;
            }
            ...
            me.$el.find('option').prop('selected', false)
        ...
        value: function(_value, trigger) {
            ...
            if (arguments.length === 0) {
                ...
                if (me.isReadonly || me.isDisabled || me.el.options.length === 0) {
                    return;
                }
        ...
        update: function(list) {
            ...
            core.each(core.toArray(me.$el[0].options), function(item, i) {
                ...
                html += '<li><a href="#' + (num++) + '" data-value="' + item.value + '" data-text="' + item.text + '" title="' + me.attrTitle + '">' + me._itemHTML(item) + '</a></li>';
            });

            if (index >= 0) {
                me.$selectboxWrapper.empty().html('<ul>' + html + '</ul>').find('li:eq(' + index + ')').addClass('on');
            } else {
                me.$selectboxWrapper.empty();
    ...
    var DateSelectbox = core.ui('DateSelectbox', {
        ...
        initialize: function(el, options) {
            ...
            if (!me.options.startTarget || !me.options.endTarget) {
                return;
            }
            ...
            /*
            $endTarget.module('calendar').on('calendarshow', function () {
            var startDate = $startTarget.dateVal();
            if (startDate && core.date.isValid(startDate)) {
            $endTarget.module('calendar').setMinDate(startDate);
            } else {
            $endTarget.module('calendar').setMinDate(null);
            }
            });
            $startTarget.module('calendar').on('calendarshow', function () {
            var endDate = $endTarget.dateVal();
            if (endDate && core.date.isValid(endDate)) {
            $startTarget.module('calendar').setMaxDate(endDate);
            } else {
            $startTarget.module('calendar').setMaxDate(null);
            }
            });
            */
        }
...
(function($, core, undefined) {
    ...
    var Modal = ui('Modal', /** @lends axl.ui.Modal# */ {
        ...
        show: function() {
            ...
            me.$scroller.find('>.ui_scrollarea').css({
                'height': '100%'
            });
            ...
            if (opts.effect === 'fade') {
                ...
                me.$el.css({
                    ...
                    top: me.forceResize ? opts.offsetTop : (winHeight - modalHeight) / 2
                }, 400, function() {
                    defer.resolve();
                });
            } else {
    ...
    $.ajaxModal = core.ui.ajaxModal = function() {
        ...
        $.ajax.apply($.ajax, core.toArray(arguments)).done(function(html) {
            $modal = $(core.string.trim(html)).addClass('ui_modal_dynamic').appendTo('body');
            $modal.scModal({
                removeOnClose: true
            }).buildUIControls();
    ...
    var BaseTab = core.ui.View.extend({
        ...
        _bindEvents: function() {
            ...
            me.$el.on('click', me.options.tabSelector + me.options.btnSelector, function(e) {
                e.preventDefault();
                if (me.getState('disabled') || me.getState('readonly')) {
                    return;
                }
        ...
        'type03': BaseTab.extend({ // 탭이 탭영역을 벗어날 경우 좌우로 스와이핑 되는 탭
            ...
            _initTab: function() {
                ...
                var move = function(val, isAni) {
                    if (isAni) {
                        me.$scroller.stop().animate({
                            'margin-left': val
                        }, function(e) {
                            enabled();
                        });
                    } else {
                        me.$scroller.stop().css({
                            'margin-left': val
                        });
                        enabled();
                    }
                ...
                var enabled = function(size) {
                    ...
                    if (!size.isOver) {
                        ...
                        me.$tabNavi.show();
                        me.$tabBox.css({
                            'margin': '0 ' + (me.$tabNavi.find('>a').width()) + 'px'
                        });
...
(function($, core, undefined) {
    ...
    var SwipeGesture = core.ui('SwipeGesture', {
        ...
        _bindSwipeEvents: function() {
            ...
            me.$el[0].onselectstart = function() {
                return false;
            };
            ...
            me.$el.on('mousedown.swipegesture, touchstart.swipegesture', function(downEvent) {
                ...
                $doc.on('mousemove.swipegesture' + me.cid + ' touchmove.swipegesture' + me.cid, function(moveEvent) {
                    ...
                    if (!isSwipe) {
                        ...
                        if ((swipeX < 1 && me.isHoriz) || (swipeY < 1 && me.isVerti)) {
                            ...
                            if (me._swipeCallback('start', touch) === false) {
                                return;
                            };
                            if (me.triggerHandler('swipegesturestart', touch) === false) {
                                return;
                            };
                            isSwipe = true;
                        } else {
                    ...
                    if (isSwipe) {
                        ...
                        touch.event = moveEvent;
                        if (me._swipeCallback('move', touch) === false) {
                            return;
                        }
                        if (me.triggerHandler('swipegesturemove', touch) === false) {
                            return;
                        }
                    }
                    ...
                    if (isSwipe && touchStart) {
                        ...
                        touch.event = upEvent;
                        if (Math.abs(touch.diff.x) > me.options.threshold || Math.abs(touch.diff.y) > me.options.threshold) {
        ...
        release: function() {
            ...
            $(document).off('.swipegesture' + me.cid);
            this.supr();
        }
    });
...
(function($, core, undefined) {
    ...
    var CheckboxAllChecker = core.ui('CheckboxAllChecker', {
        ...
        _bindEvents: function() {
            ...
            me.on('checkedchanged', function(e) {
                //me.$wrapper.find(selector).not(this).checked(this.checked);
                if (me.$wrapper[0].querySelectorAll) {
                    $(me.$wrapper[0].querySelectorAll('[type=checkbox]'))
                        .filter(':enabled:not(.ui_checkall_ignore)').checked(this.checked);
                } else {
...
(function($, core, undefined) {
    ...
    var contains = function(parentEl, el) {
        ...
        if (parentEl === el || $.contains(parentEl, el)) {
            return true;
        }
        return false;
    };
 ...
 * @create 2014-12-08
 * @license MIT License
 * @description 액슬카드 전용 유틸함수 모음
 */
(function($, core, undefined) {
    "use strict";
    if (core.ui.isNumberKeys) {
        return;
    }

    core.ui.isNumberKeys = function(keyCode) {
        return (keyCode >= 48 && keyCode <= 57) || (keyCode >= 96 && keyCode <= 105) // 오른쪽 숫자키
            || (keyCode === 37) // left
            || (keyCode === 39) // right
            || (keyCode === 9) // left
            || (keyCode === 46) // delete
    ...
    core.ui.setDefaults('Accordion', {
        on: {
            'accordionexpand': function(e, data) {
                var $el = $(this),
                    instance = $el.scAccordion('instance'),
                    $scrollview;
    ...
    $(function() {
        core.importJs([
            ...
            setInterval(function() {
                ...
                for (var i = 0; i < len; i++) {
                    input = inputs[i];
                    if ((input.type === 'text' || input.type === 'password') && input.className.indexOf('ui_formatter') >= 0 && !input.getAttribute('data-formatted')) {
                        $(input).scFormatter();
