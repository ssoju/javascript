...
(function ($, core, undefined) {
     ...
     * @class
     * @name scui.ui.TextControl
     * @extends scui.ui.View
     * @fires scui.ui.TextControl#textcontrolchange
     * @fires scui.ui.TextControl#textcontrolresize
     * @example
     * new ui.TextControl( $('textarea'), {checkCount: true});
     * // or
     * $('textarea').scTextControl({checkCount: true});
     */
    var TextControl = ui('TextControl', /** @lends scui.ui.TextControl# */{
        $statics: {
            ON_INIT: 'init',
        ...
        initialize: function (el, options) {
            ...
            me.$el.on('keydown keyup cut paste blur userpaste', function(e) {
                var isOver = me._checkLimit();
                if (e.type === 'keyup' || e.type === 'paste' || e.type === 'userpaste') {
                    if (isOver) {
                        core.showMessage('N0000004'); // 입력하신 글자 수가 초과되었습니다.
                        this.focus();
                    }
                }
                /**
                 * 텍스트가 변경됐을 때 발생
                 * @event scui.ui.TextControl#textcontrolchange
                 * @type {Object}
	        ...
	        if (core.browser.isIOS) {
		        // ios safari에서 paste이벤트가 인라인만 먹고 있어서 아래와 같이 처리함
		        me.$el.attr('onpaste', 'var el = this;setTimeout(function () { $(el).triggerHandler("userpaste"); })');
	        }
            me._checkLimit();
            me.trigger('textcontrolchange', {length: me.currentLength});
        ...
        _initTextControl: function () {
            ...
            if (!o.allowPaste) {
                me.on('paste', function (e) {
	                ...
	                core.showMessage('N0000004'); // 죄송합니다. \n도배글 등을 방지하기 위해 붙여넣기를 하실 수 없습니다.
                });
            }
        ...
        _layout: function(me, $el, $clone, initialHeight, offset) {
             ...
             * @event scui.ui.TextControl#textcontrolresize
             */
    ...
    TextControl.prototype.defaults.countText = '<span class="fc_dgray">{len}</span>/{limit}자';
