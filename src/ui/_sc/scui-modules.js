...
(function ($, core, undefined) {
	...
	var ModalManager = window.ModalManager = {
		...
		init: function (options) {
			...
			window.ui_slink = false; // 160523 link 페이지 전역변수 리셋 추가
				...
				if ($next.size() > 0) {
					...
					if (target = ($el.attr('href') || $el.attr('data-target') || $el.attr('data-href'))) {
						...
						if (!/^#/.test(target)) {
							...
							if (window.scard && $el.data('crossDomain') && scui.browser.isIE && scui.browser.ieVersion < 10  ) {
							...
							}if(window.ui_slink) { // 160523 link 페이지 추가
								
								if($el.is(':not(.linked)') && $el.is(':not(.vt_link)')){ //링크가능
									//var img_tmp = $el.find('.btns>img').attr('src');
									//var img_path = img_tmp.substring(0, img_tmp.lastIndexOf('/')+1); 
									var img_path = '/images/personal/benefits/';

									var idx = 5;
									$el.find('.btns').html('<img src="/images/personal/benefits/action_benefits05.png" />');

									me.linkInterval = setInterval(function(){
										if(idx == 0){
											clearInterval(me.linkInterval);
											...
											$el.find('.btns').html('<span>카드에 <strong>링크됨</strong></span>');
										}else{
											$el.find('.btns>img').attr('src', img_path + 'action_benefits' + (idx < 10 ? '0'+idx:idx) + '.png');
											idx --;
										}
									}, 100);

								}else{ // 160523 link 페이지 추가 끝
									// 이미 링크됨
								...
								$.ajax({
									...
									$modal.addClass('ui_modal_dynamic').buildUIControls().scModal('.ui_modal_dynamic',$.extend({
                                        removeOnClose: true
                                    }, $el.data(), {opener: $el})).one('modalhidden', function (e) {
                                        setTimeout(function (){ $el[0].focus(); });
                                    });
	...
	var SwipeGesture = core.ui('SwipeGesture', {
		...
		_bindSwipeEvents: function() {
			...
			me.$el.on('mousedown.swipegesture, touchstart.swipegesture', function(downEvent) {
				...
				if (downEvent.type === 'mousedown' || downEvent.type === 'touchstart') {
...
(function ($, core) {
	...
	function createMenuLink(item) {
		...
		return '<li '+(item.focus_yn == "T"?  'class="ico_link"':'')+'><a href="'+ item.link + '" data-menu-id="' + ((item.copy_yn !== 'T') ? linkParse(item.link) : '') + '"' +
	...
	function createMenu(item, isUtilMenu) {
		...
		core.each(item.child, function (twoItem, j) {
			...
			if (hasChild) {	// 3depth
				...
				html += '<li><a href="#" class="open_link' +(twoItem.focus_yn == "T"?  ' ico_link':'') +'">' + twoItem.name + '<span class="ico_open">' +
		...
		renderByJson: function (data) {
			...
			if (menuList = rootMenu.child) {
				...
				// 160607 메뉴 추가
				 ...
				 if (focusItem) {
                    var preHtml = '<div class="top_link"><a href="'+focusItem.link+'">'+ focusItem.description +'</a></div>';        
                    $('.allmenu_wrap').prepend(preHtml);                                
                }
			...
			if (menuList = rootMenu.child) {
				core.each(menuList, function (oneItem, i) {
					...
					if (oneItem.menu_type === 'util') {						
						...
						if(oneItem.focus_yn === 'T'){
						    htmlUtil += '<div class="menu_list ss_view ui_menu_list">';
                            htmlUtil += '<h2><a href="'+ oneItem.link +'"><span class="ico"></span>'+oneItem.name+'</a></h2></div>';                            
						}else{
	...
	var Gnb = core.ui('Gnb', {
		...
		_bindEvents: function () {
			...
			// 현재 보이고 있는 요소에 .visible 클래스 추가
			$win.on('changemediasize.'+me.cid, fn = function (e) {
			...
			if ($banner.size() > 0) {
				...
				setTimeout(function(){
					if (core.Cookie.get('main_banner_hide') !== 'Y') {
						...
						me.$('.top_banner').slideDown(500, 'easeInOutCirc', function(){}).addClass('visible');
					}
				}, 1000);
				...
				me.$el.on('click', '.ui_banner_close', function (e) {
					e.preventDefault();
					me.$('.top_banner').slideUp(800, 'easeInOutCirc', function(){
				...
				// 오늘하루 열지 않기
				me.$el.on('click', '.ui_banner_cancel', function (e) {
					e.preventDefault();
					me.$('.top_banner').slideUp(800, 'easeInOutCirc', function(){
						$(this).removeClass('visible');
