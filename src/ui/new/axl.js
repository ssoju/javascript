    ...
    $.fn.toggleLayer = function () {
        return this.each(function () {
            ...
            $el.on('click', function (e) {
                ...
                $target.toggle(!$target.is<http://target.is>(':visible'));
            });
        });
    };
...
// 표시될 때 d_open 클래스 추가
    ...
    $.fn.hideLayer = function (options, isTrigger) {
        ...
        return this.each(function () {
            ...
            if (isTrigger !== false){ $this.trigger($.fn.hideLayer.ON_HIDDEN); }
...
// 숨겨진 후에 열었던 원래버튼에 포커스를 강제로 준다.
    ...
    extend(core, {
         ...
         * 타입 체크
         * @function
         * @name axl.is<http://axl.is>
         * @param {Object} o 타입을 체크할 값
         * @param {string} typeName 타입명(null, number, string, element, nan, infinity, date, array)
         * @return {boolean}
         * @example
         * axl.is<http://axl.is>('aaaa', 'string'); // true
         * axl.is<http://axl.is>(new Date(), 'date'); // true
         * axl.is<http://axl.is>(1, 'number'); // true
         * axl.is<http://axl.is>(/[a-z]/, 'regexp'); // true
         * axl.is<http://axl.is>(document.getElementById('box'), 'element'); // true
         * axl.is<http://axl.is>({a:'a'}, 'object'); // true
         * axl.is<http://axl.is>([], 'array'); // true
         * axl.is<http://axl.is>(NaN, 'nan'); // true
         * axl.is<http://axl.is>(null, 'null'); // true
         * // 파라미터를 하나만 넘기면 타입명을 반환받을 수 있다.
         * axl.is<http://axl.is>('') // "string"
         * axl.is<http://axl.is>(null) //"null"
         * axl.is<http://axl.is>(1) //"number"
         * axl.is<http://axl.is>({}) //"object"
         * axl.is<http://axl.is>([]) // "array"
         * axl.is<http://axl.is>(undefined) // "undefined"
         * axl.is<http://axl.is>(new Date()) // "date"
         * axl.is<http://axl.is>(/[a-z]/) // "regexp"
         * axl.is<http://axl.is>(document.body) //"element"
         */
        is: isType,
        /**
         * 타입 체크 axl.is<http://axl.is>의 별칭
         * @function
         * @name axl.type
         * @param {Object} o 타입을 체크할 값
         * @param {string} typeName 타입명(null, number, string, element, nan, infinity, date, array)
        ...
        isEmpty: function (value, allowEmptyString) {
            return (value === null)
                || (value === undefined)
                || (value === 0)
                || (core.is<http://core.is>(value, 'string') && !allowEmptyString ? value === '' : false)
            || (core.is<http://core.is>(value, 'array') && value.length === 0)
            || (core.is<http://core.is>(value, 'object') && !core.object.hasItems(value));
        },
        ...
        return /** @lends axl.string */{
            ...
            format: function (format, val) {
                var args = core.toArray(arguments).slice(1),
                    isJson = core.is<http://core.is>(val, 'object');
 ...
 * });
         ...
         */
        map: function (obj, cb) {
            if (!core.is<http://core.is>(obj, 'object') || !core.is<http://core.is>(cb, 'function')) {
            return obj;
        }
        var results = {};
    each(obj, function (v, k) {
 ...
 */
    ...
    if (!core.is<http://core.is>(obj, 'object')) {
    return false;
}
...
remove: function (value, key) {
    if (!core.is<http://core.is>(value, 'object')) {
    return value;
}
value[key] = null;
delete value[key];
...
map: nativeCall(arrayProto.map) || function (obj, cb, ctx) {
    var results = [];
    if (!core.is<http://core.is>(obj, 'array') || !core.is<http://core.is>(cb, 'function')) {
    return results;
}
// vanilla js~
for (var i = 0, len = obj.length; i < len; i++) {
...
every: nativeCall(arrayProto.every) || function (arr, cb, ctx) {
    var isTrue = true;
    if (!core.is<http://core.is>(arr, 'array') || !core.is<http://core.is>(cb, 'function')) {
    return isTrue;
}
...
include: function (arr, value, b) {
    if (!core.is<http://core.is>(arr, 'array')) {
    return value;
}
 ...
 */
    ...
    if (!core.is<http://core.is>(value, 'array')) {
    return value;
}
...
core.addon('date', function () {
    ...
    return /** @lends axl.date */{
        ...
        format: function (formatDate, formatString) {
            ...
            if (core.is<http://core.is>(formatDate, 'number')) {
            formatDate = new Date(formatDate);
        } else if (core.is<http://core.is>(formatDate, 'string')) {
...
parse: (function () {
    ...
    return function (dateStringInRange) {
        ...
        if (dateStringInRange.length !== 8) { return new Date(NaN); }
 ...
 */
diff: function (t1, t2) {
    if (!core.is<http://core.is>(t1, 'date')) {
    t1 = new Date(t1);
}

if (!core.is<http://core.is>(t2, 'date')) {
t2 = new Date(t2);
}
...
core.addon('uri', /** @lends axl.uri */{
    ...
    addParam: function (url, string) {
        if (core.is<http://core.is>(string, 'object')) {
 ...
 * // {scheme: "http", host: "www.axl.com<http://www.axl.com>", port: "8080", path: "/list.do", query: "a=1&b=2"…}
 */
parseUrl: (function () {
    var o = {
        strictMode: false,
...
// 캐럿 위치 반환
    getCaretPos: function(el) {
        if(core.is<http://core.is>(el.selectionStart, 'number')) {
        return {
...
setCaretPos: function(el, pos) {
    if(!core.is<http://core.is>(pos, 'object')) {
    pos = {
        begin: pos,
        end: pos
    };
 ...
 */
    ...
    if (!core.is<http://core.is>(name, 'string')) {
...
// 속성 중에 부모클래스에 똑같은 이름의 함수가 있을 경우 래핑처리
    ...
    function classExtend(attr, c) {
        ...
        if (core.is<http://core.is>(attr, 'function')) {
        attr = attr();
    }
...
// supr = supr;
        ...
        if (core.is<http://core.is>(attr, 'function')) {
        if (!core.is<http://core.is>(attr = attr(supr), 'function')) {
...
var execObject = function (obj, ctx) {
    return core.is<http://core.is>(obj, 'function') ? obj.call(ctx) : obj;
};
...
// }
        ...
        core.each(me.options.events, function (value, key) {
            ...
            var name = RegExp.$1,
                selector = RegExp.$2,
                args = [name],
                func = core.is<http://core.is>(value, 'function') ? value : (core.is<http://core.is>(me[value], 'function') ? me[value] : core.emptyFn);
...
// me에 등록된 엘리먼트들의 연결고리를 해제(메모리 해제대상)
    ...
    setState: function (name, flag) {
        ...
        this.triggerHandler('statechange', {name: name, value: flag});
    }
});
...
ui.bindjQuery = function (Klass, name, prefix) {
    ...
    $.fn[pluginName] = function (options) {
        ...
        this.each(function () {
            ...
            if (typeof options === 'string' && core.is<http://core.is>(instance[options], 'function')) {
...
//var winCoords = axl.util.popupCoords(width, height),
        ...
        if (core.is<http://core.is>(imgs, 'string')) {
        imgs = $(imgs);
    }
    var me = this,
        defer = $.Deferred(),
...
getDimensions: function (elem) {
    if (core.is<http://core.is>(elem, 'string')) {
    elem = $(elem);
}
...
getDeltaY: function (e) {
    ...
    var detail;
    if ('detail' in e) { detail = e.detail * 1; }
    if ('wheelDelta' in e) { detail = e.wheelDelta; }
    if ('wheelDeltaY' in e) { detail = e.wheelDeltaY; }
    if ('deltaY' in e) { detail = e.deltaY * -1; }
...
// 현재 창 사이즈가 모바일 사이즈인지 여부
    ...
    !core.browser.placeholder && (function () {
        ...
        $.fn.val = function () {
            if (arguments.length === 0) {
                if (this.get(0).value === this.attr('placeholder')) {
                    return '';
                }
            }
            return oldVal.apply(this, [].slice.call(arguments, 0));
        };
        /*if (IS_DEBUG && !supportPlaceholder) {
         var oldVal = $.fn.val;
         $.fn.val = function () {
         if (arguments.length === 0) {
         if (oldVal.call(this) === this.attr('placeholder')) {
         return '';
         }
         }
         return oldVal.apply(this, [].slice.call(arguments, 0));
...
// 주어진 값을 포맷화해서 value에 설정
    ...
    $.fn.checked = function (checked, isTrigger) {
        return this.each(function () {
            ...
            if (this.disabled) { $(this).parent().addClass('disabled'); return; }
...
// 포커싱에 따라 x버튼 토글
                $that.on('focusin.globalui focusout', function (e) {
                    var isNotEmpty = $inp.val() !== '';
                ...
                }).triggerHandler('focusin.globalui');
            });
        },
            ...
            $doc.on('mouseenter.globalui mouseleave.globalui focusin.globalui focusout.globalui',
                ...
                function (e) {
                    ...
                    switch (e.type) {
                        case 'mouseenter':
                        case 'focusin':
                            if (!$el.hasClass('on')) { $el.addClass('active'); }
                            break;
                        default:
                ...
                switch (true) {
                    ...
                    case $el.hasClass('ui_close'): // 닫기버튼
                        if (me.isPopup) { // 팝업 닫기
                            ...
                            if (!target) { target = '.ui_modal_layer'; }
...
// TODO 나중에 서버 개발시 뺄것 - 퍼블리싱에서 임시로 보기 위해 만든 코드
// PC/Mobile CSS 구분 로딩 START - 나중에 제거
