    ...
    var SlideBar = ui.View.extend( /**@lends emart.ui.SlideBar# */ {
        ...
        _initEvents: function() {
            ...
            me.on('mousedown touchstart', function(e) {
                ...
                if(me.options.maxValue <= 0) { return; }
                if(me.$el.hasClass('disabled')) { return; }
                if(pos.x !== upPos.x || pos.y !== upPos.y) { return; }
        ...
        _bindDrag: function () {
            ...
            dragger.on('dragstart', function(e, data) {
                ...
                if(x >= 0 && x <= trackSize) {
                    ...
                    me.triggerHandler('valuechange', {value: me.options.maxValue * per});
                }
            ...
            if (!isTouch) {
                ...
                me.on('keydown', function(e) {
                    if (e.keyCode === me.downKey || e.keyCode === me.upKey) { // <> 38:up, 40: down
                        ...
                        if ((+new Date) - lastTime < 200) { return; }
        ...
        setValue: function(value) {
            ...
            if (value < me.options.minValue
                || value > me.options.maxValue
                || me.$bar.hasClass('d-dragging')) {
                return;
            }
    ...
    var PlayList = ui.View.extend(/** @lends emart.ui.PlayList# */{
        ...
        initialize: function(el, options) {
            ...
            me.$list.on('click', 'a', function(e) {
                ...
                if(!confirm("'"+$li.find('.tit').text()+"'을(를) 삭제하시겠습니까?")) { return; }
        ...
        addRow: function(data) {
            ...
            //접근성 수정 160707
            //me.$list.append(['<li class="d-track" data-id="' + data.id + '">' +
            //'<input type="checkbox" name="" value="' + data.id + '" id="track-'+data.id+'" title="' + data.title + ' 곡 선택하기">',
            //    '<label for="track-'+data.id+'" class=""><span class="tit"><a href="#" data-id="' + data.id + '">'+data.title+'</a></span> - <span class="pd">' + data.artist + '</span></label>' +
            //    '<button class="list_del"><span class="none">곡 삭제하기</span></button>' +
            //    '</li>'].join(''));
            me.$list.append(['<li class="d-track" data-id="' + data.id + '">' +
            '<input type="checkbox" name="" value="' + data.id + '" id="track-'+data.id+'" title="' + data.title + ' 곡 선택하기">',
                '<label for="track-'+data.id+'" class=""><span class="tit">'+data.title+'</span> - <span class="pd">' + data.artist + '</span></label>' +
                '<a href="#lyrics" data-id="' + data.id + '" class="btn_musiclyrics">가사보기</a>' +
                '<button class="list_del"><span class="none">곡 삭제하기</span></button>' +
                '</li>'].join(''));
        },
    ...
    var AudioPlayer = ui('EmartAudioPlayer', /**@lends emart.ui.EmartAudioPlayer*/ {
        ...
        _initEvents: function() {
            ...
            me.on('click', '.d-play', function(e) {
                ...
                var $el = $(this);
                if($el.hasClass('on')){ $el.removeClass('on'); me.options.loop = ''; }
                else if($el.hasClass('one')){ $el.replaceClass('one', 'on'); me.options.loop = 'all'; }
                else {  $el.addClass('one'); me.options.loop = 'one'; }
            ...
            me.playList.on('addedtrack', function(e, data) {
                ...
                // 곡추가
                !me.isPlaying() && me._setTrackInfo({title: '총 ' + me.playList.tracks.size() + '곡이 있습니다.'});
        ...
        _generateTime: function(time) {
            ...
            return time | 0 > 0 ?
                mejs.Utility.secondsToTimeCode(time,
                    false,
                    false,
        ...
        setSrc: function(url) {
            ...
            if(url){ // 캐쉬와 관련된 문제가 있어서 매번 새로 가져오게끔 시간값을 추가
                url = core.uri.addParam(url, {_: +(new Date())});
            } else {
                me.pause();
            }
            me.media.setSrc(url);
